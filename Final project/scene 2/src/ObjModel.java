...
public class ObjModel {
    ...
    /**
     * Adds the model lying flat on the floor (rotates 90° around the X axis).
     * Model XY plane → world XZ plane. Useful for flat logos/emblems.
     * offset.y = floor height, offset.x/z = center position.
     */
    public void addToSceneFloor(Scene scene, Material defaultMat, Vector3D offset, double scale) {
        for (int i = 0; i < triV.size(); i++) {
            int[] vi  = triV.get(i);
            int[] vni = triVN.get(i);

            Vector3D a = xformFloor(vertices.get(vi[0]), offset, scale);
            Vector3D b = xformFloor(vertices.get(vi[1]), offset, scale);
            Vector3D c = xformFloor(vertices.get(vi[2]), offset, scale);

            Vector3D pn0 = null, pn1 = null, pn2 = null;
            if (!normals.isEmpty() && vni[0] >= 0 && vni[1] >= 0 && vni[2] >= 0) {
                pn0 = xformNormalFloor(normals.get(Math.min(vni[0], normals.size()-1)));
                pn1 = xformNormalFloor(normals.get(Math.min(vni[1], normals.size()-1)));
                pn2 = xformNormalFloor(normals.get(Math.min(vni[2], normals.size()-1)));
            }

            scene.addObject(new Triangle(a, b, c, pn0, pn1, pn2, defaultMat, false));
        }
    }

    // 90° X-axis rotation: model (x, y, z≈0) → world (x, z, -y)
    private Vector3D xformFloor(Vector3D v, Vector3D off, double s) {
        return new Vector3D(
             v.x * s + off.x,   // X stays the same
             v.z * s + off.y,   // Z of model (thickness ≈0) → world Y
            -v.y * s + off.z    // Y of model (logo depth) → -Z
        );
    }

    private Vector3D xformNormalFloor(Vector3D n) {
        return new Vector3D(n.x, n.z, -n.y).normalize();
    }
