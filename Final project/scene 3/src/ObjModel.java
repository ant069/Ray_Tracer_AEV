...
public class ObjModel {
    ...
    /**
     * Adds the model to the scene with a Y-axis rotation applied before
     * scale and offset. Useful for orienting models that arrive in the wrong
     * plane (e.g., the window frame needs -90° to stand perpendicular to
     * the left wall of the shack).
     *
     * @param rotY rotation angle in radians (positive = counter-clockwise from above)
     */
    public void addToScene(Scene scene, Map<String, Material> matMap,
                           Material defaultMat, Vector3D offset, double scale, double rotY) {
        ...
        for (int i = 0; i < triV.size(); i++) {
            ...
            Vector3D a = rotXform(vertices.get(vi[0]), offset, scale, cosY, sinY);
            Vector3D b = rotXform(vertices.get(vi[1]), offset, scale, cosY, sinY);
            Vector3D c = rotXform(vertices.get(vi[2]), offset, scale, cosY, sinY);

            Vector3D pn0 = null, pn1 = null, pn2 = null;
            if (vni[0] >= 0 && vni[1] >= 0 && vni[2] >= 0 && !normals.isEmpty()) {
                pn0 = rotNorm(normals.get(Math.min(vni[0], normals.size()-1)), cosY, sinY);
                pn1 = rotNorm(normals.get(Math.min(vni[1], normals.size()-1)), cosY, sinY);
                pn2 = rotNorm(normals.get(Math.min(vni[2], normals.size()-1)), cosY, sinY);
            }
    ...
    /** Applies Y rotation + scale + offset to a vertex. */
    private Vector3D rotXform(Vector3D v, Vector3D off, double s, double cosY, double sinY) {
        double rx = v.x * cosY + v.z * sinY;
        double ry = v.y;
        double rz = -v.x * sinY + v.z * cosY;
        return new Vector3D(rx * s + off.x, ry * s + off.y, rz * s + off.z);
    }

    /** Applies Y rotation only to a normal (no scale or offset). */
    private Vector3D rotNorm(Vector3D n, double cosY, double sinY) {
        double rx = n.x * cosY + n.z * sinY;
        double rz = -n.x * sinY + n.z * cosY;
        return new Vector3D(rx, n.y, rz);
    }
